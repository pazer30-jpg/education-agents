...
Two background services keep the system alive between pipeline runs:

| Job | Frequency | Purpose |
|---|---|---|
| `com.paz.moki.autonomy` | hourly, on the hour | Run autonomy.py — each agent's daily routine, dispatched by hour |
| `com.paz.moki.telegram-poll` | every 5 min | Drain Telegram button clicks (✅/✏️/❌) on ready posts |

## Install

```bash
cp /Users/ASUS/education-agents/launchd/com.paz.moki.autonomy.plist      ~/Library/LaunchAgents/
cp /Users/ASUS/education-agents/launchd/com.paz.moki.telegram-poll.plist ~/Library/LaunchAgents/

launchctl load ~/Library/LaunchAgents/com.paz.moki.autonomy.plist
launchctl load ~/Library/LaunchAgents/com.paz.moki.telegram-poll.plist
```
...
launchctl unload ~/Library/LaunchAgents/com.paz.moki.telegram-poll.plist
rm ~/Library/LaunchAgents/com.paz.moki.autonomy.plist
rm ~/Library/LaunchAgents/com.paz.moki.telegram-poll.plist
```
